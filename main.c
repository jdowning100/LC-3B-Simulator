...
void AGEX_stage() {
    ...
    int addop1 = 0;
    int addop2 = 0;
    int new_mem_address = 0;
    int new_mem_alu_result = 0;
    switch(Get_ADDR1MUX(PS.AGEX_CS)){

        case 0: addop1 = PS.AGEX_NPC;
            break;
        case 1: addop1 = PS.AGEX_SR1;
            break;
    }
    switch(Get_ADDR2MUX(PS.AGEX_CS)){
        ...
        case 0: /*ZERO*/
            ...
            break;
        case 1: /*offset6*/
            addop2 = signExtension_6(PS.AGEX_IR & 0x1F);
            break;
        case 2: /*PCoffset9*/
            addop2 = signExtension_9(PS.AGEX_IR & 0x1FF);
            break;
        case 3: /*PCoffset11*/
            addop2 = signExtension_11(PS.AGEX_IR & 0x7FF);
            break;
    }
    ...
    switch(Get_ADDRESSMUX(PS.AGEX_CS)){
        ...
        case 0: new_mem_address = addop1 + addop2;
            break;

        case 1:new_mem_address = (PS.AGEX_IR & 0xFF) << 1;
            break;
    }

    int amount = PS.AGEX_IR & 0x0F;
    int shift = 0;
    if(((PS.AGEX_IR >> 4) & 0x03) == 0){           /*LSHF*/
        shift = Low16bits(PS.AGEX_SR1 << amount);
    }
    else if(((PS.AGEX_IR >> 4) & 0x03) == 1){
        uint16_t temp = PS.AGEX_SR1 >> amount;     /*gcc should perform logical right shift on unsigned int*/
        shift = temp;
    }
    else if(((PS.AGEX_IR >> 4) & 0x03) == 3){
        int16_t temp1 = PS.AGEX_SR1;
        int16_t temp2 = temp1 >> amount;
        shift = Low16bits(temp2);     /*gcc should perform arithmetic right shift on signed int*/
    }

    int operand = 0;

    switch(Get_SR2MUX(PS.AGEX_CS)){
        case 0: operand = PS.AGEX_SR2;
            break;
        case 1: operand = signExtension_5(PS.AGEX_IR & 0x1F);
            break;
    }
    ...
    switch(Get_ALUK(PS.AGEX_CS)){
        case 0: /*ADD*/
                alu = Low16bits(PS.AGEX_SR1 + operand);
                break;
    ...
    switch(Get_ALU_RESULTMUX(PS.AGEX_CS)){
        case 0: new_mem_alu_result = shift;
            break;
        case 1: new_mem_alu_result = alu;
    }
    ...
    if (LD_MEM) {
        ...
        NEW_PS.MEM_ALU_RESULT = new_mem_alu_result;
        NEW_PS.MEM_ADDRESS = new_mem_address;
...
void DE_stage() {
    ...
    int SR1 = (PS.DE_IR & 0x1C0) >> 6;
    int SR2;
    switch((PS.DE_IR & 0x2000) >> 13){
        case 0: SR2 = PS.DE_IR & 0x07;
            break;
        case 1: SR2 = (PS.DE_IR >> 9) & 0x07;
    }

    int DR;

    switch(Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS])) {
        case 0:
            DR = (PS.DE_IR >> 9) & 0x07;
            break;
        case 1:
    ...
    if((SR1 == PS.AGEX_DRID) && Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && v_agex_ld_reg)
        dep_stall = 1;
    else if((SR1 == PS.MEM_DRID) && Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && v_mem_ld_reg)
        dep_stall = 1;
    else if((SR1 == PS.SR_DRID) && Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && v_sr_ld_reg)
        dep_stall = 1;
    else if((SR2 == PS.AGEX_DRID) && Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && v_agex_ld_reg)
        dep_stall = 1;
    else if((SR2 == PS.MEM_DRID) && Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && v_mem_ld_reg)
        dep_stall = 1;
    else if((SR2 == PS.SR_DRID) && Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && v_sr_ld_reg)
        dep_stall = 1;
    else if(Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (Get_AGEX_LD_CC(PS.AGEX_CS) || Get_MEM_LD_CC(PS.MEM_CS) || Get_SR_LD_CC(PS.SR_CS)))
        dep_stall = 1;
    else
        dep_stall = 0;
    ...
    if (LD_AGEX) {
        ...
        NEW_PS.AGEX_CC = (sr_n << 2) + (sr_z << 1) + sr_p;
        NEW_PS.AGEX_DRID = DR;
...
/************************* FETCH_stage() *************************/
void FETCH_stage() {
    icache_access(PC, &read_instruction, &icache_r);
    int LD_DE;
    int NPC;
    if (!icache_r)
        NEW_PS.DE_V = 0;
    else {
        NEW_PS.DE_V = !(v_agex_br_stall || v_mem_br_stall);
        switch (mem_pcmux) {
            case 0:
                NPC = PC + 2;
                break;
    ...
    if(dep_stall || mem_stall || v_de_br_stall || v_agex_br_stall || v_mem_br_stall || !icache_r)
        ...
        LD_DE = 1;
    if (LD_DE) {
        NEW_PS.DE_V = !(v_de_br_stall || v_agex_br_stall || v_mem_br_stall);
        NEW_PS.DE_IR = read_instruction;
        NEW_PS.DE_NPC = NPC;
        if(NEW_PS.DE_V && mem_pcmux == 0)
            PC = NPC;
...
int signExtension_5(int instr) {
    int value = (0x1F & instr);
    int mask = 0x10;
    int sign = (mask & instr) >> 4;
    if (sign == 1)
        value += 0xFFE0;
    return value;
}

int signExtension_9(int instr) {
    int value = (0x1FF & instr);
    int mask = 0x100;
    int sign = (mask & instr) >> 8;
    if (sign == 1)
        value += 0xFE00;
    return value;
}

int signExtension_11(int instr){
    int value = (0x7FF & instr);
    int mask = 0x400;
    int sign = (mask & instr) >> 10;
    if (sign == 1)
        value += 0xF800;
    return value;
}

int signExtension_6(int instr){
    int value = (0x3F & instr);
    int mask = 0x20;
    int sign = (mask & instr) >> 5;
    if (sign == 1)
        value += 0xFFC0;
    return value;
}

int signExtension_8(int instr){
    int value = (0xFF & instr);
    int mask = 0x80;
    int sign = (mask & instr) >> 7;
    if (sign == 1)
        value += 0xFF00;
    return value;
}
